...
/**
 * Spacecraft component displays detailed information about a specific spacecraft.
 *
 * @returns {JSX.Element} The rendered Spacecraft component.
 */
function Spacecraft() {
  ...
  useEffect(() => {
    /**
     * Fetch spacecraft data by ID and update the component state.
     */
    async function getSpacecraft() {
      enableLoading();
      const { data: fetchedSpacecraft, isError } =
        await SpaceTravelApi.getSpacecraftById({ id });
      if (!isError) {
        setSpacecraft(fetchedSpacecraft);
      }
      disableLoading();
    }
